...
async def get_url(data):
        ...
        try:
            if Status[0] != 'пропустити':
                for id in range(len(Status)):
                    ...
                    #     print('status', Status[id] in status)
                    #     filters += f'&status%5B{id}%5D={status[Status[id]]}'
                    # else:
                    #     print("Invalid status")
        except KeyError as error:
            list_url.append({'error': f"Вітаю ви мали отримати список тендерів, але ви не вірно вказали даний параметр - {error}."
    ...
    #     # return (f"Вітаю ви мали отримати список тендерів, але ви не вірно вказали даний параметр - {error}."
    #     #         f",\nБудь ласка створить новий запит, з виправленим значенням.")
